...
※ `cargo test` が通っても AE 上の挙動確認は別です。挙動変更がある場合は `tester/` の .aep で確認するか、同等の手動確認手順と実結果を残してください。

### 4.1 不具合調査の進め方（事実ベース）

- 修正前に **再現手順 / 期待値 / 実際の結果 / 実行コマンド or ログ** を先に固定する。
- CI 不一致は、まず失敗したジョブとローカル実行コマンドを 1 対 1 で対応づけて切り分ける。
- 「たぶんこれが原因」で広く触らず、最小の再現条件と最小の変更で確認する。
